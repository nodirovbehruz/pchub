...
6. **Realtime/ЧАТ (важно — на сервере чат не работал):** ответ оператора доставляется
   клиенту ТОЛЬКО по WebSocket (`push_chat_to_user`), REST-фолбэка у шелла нет.
   **КОРНЕВАЯ ПРИЧИНА (подтверждена прогоном шелла + аудитом):** прод поднимался как
   `gunicorn settings.wsgi:application` (`docker/start.sh`) — это **WSGI**, который
   физически не умеет WebSocket-upgrade. Хендшейк `/ws/client/` отдавал **HTTP 404**
   (роут есть только в ASGI `websocket_urlpatterns`), поэтому чат/баланс/команды НЕ
   работали вообще, независимо от channel layer. Лог шелла: `[RT] connection error:
   server returned status code '404' when status code '101' was expected`.
   **Фиксы в коде (этот проход):**
   - `docker/start.sh`: `gunicorn settings.wsgi:application` → `gunicorn
     settings.asgi:application -k uvicorn.workers.UvicornWorker` (HTTP+WS из одного
     приложения, 4 воркера сохранены).
   - `requirements.txt`: добавлен `uvicorn[standard]==0.34.0` (ASGI-воркер не был
     установлен вообще — только gunicorn).
   - channel layer уже авто-использует `REDIS_URL`/`REDIS_CHANNELS_URL` (прошлый проход)
     — обязательно при >1 воркере, иначе `group_send` не пересекает воркеры.
   - `.env.example`: `DJANGO_SETTINGS_MODULE=settings` (было `settings.settings` —
     неверно) и `REDIS_CHANNELS_URL` указывает на сервис `redis`, а не `127.0.0.1`.
   **ВЕРИФИКАЦИЯ (red→green, локально):** под `uvicorn settings.asgi:application` WS-проба
   `/ws/client/` без токена → **HTTP 403** (консьюмер достигнут, корректно отверг по
   отсутствию токена) вместо 404. До правки = 404, после = 403 → роут обслуживается.
   **На сервере ещё нужно (НЕ код — env/деплой):** в проде реальный `.env` должен
   содержать `REDIS_URL`/`REDIS_CHANNELS_URL` (сейчас там только DB-переменные → channel
   layer падает в InMemory). После `git pull` + пересборки контейнера + рестарта:
   `redis-cli ping` → PONG; в логах нет ошибок Redis; WS отдаёт 101, а не 404.
   ⚠️ Альтернативный путь `docker/supervisord.conf` (`uwsgi`) — тоже WSGI; если деплой
   когда-нибудь пойдёт через него, заменить на ASGI-сервер аналогично.
...
- **🔴 Самообслуживание «купи тариф с баланса» (2026-06-18, по прогону шелла):** клиент
  входил с деньгами на депозите, но без купленного времени (минут=0). Бэк считал
  `has_access = (минут>0)`, **игнорируя депозит**, → отдавал `has_access=false` → шелл
  трактовал как «время вышло», убивал процессы и через 5 сек выкидывал на логин. То есть
  оплативший клиент НЕ мог сам купить часы, хотя деньги были. **Фикс:** (бэк)
  `check_user_access` теперь возвращает `deposit_money` + `can_buy_tariff` (есть ли
  активный тариф, который покрывает депозит); (шелл) при `has_access=false && can_buy` —
  не выход, а авто-открытие экрана тарифов с баннером «У вас N сум — выберите тариф».
  Выход остаётся только когда и времени нет, и денег нет. Регресс: 3 теста
  (`tests/test_can_buy_tariff.py`), вся сюита 59 зелёных; `dotnet build` шелла 0 ошибок.
  Сама покупка (`TariffsContentViewModel` → `ClientBuyTariffAPIView`) уже работала —
  чинили только то, что клиента выгоняли раньше времени. End-to-end в шелле требует
  стенда (клуб на проде + клиент с депозитом>0, минут=0).
...
### Косметика фронт/шелл + Combos — ЗАКРЫТО (агенты, после сброса лимита, build ✓)
- Фронт: Combos-форма (тариф/зона/товары), формат денег/дат Orders, Gantt у полуночи +
  REDEEMED-слоты + брони через полночь, DISABLED-ПК красным, капы клубов/клиентов,
  поиск логов по оператору, кламп процентов + unsaved-guard Settings, карточка нового клуба.
- Шелл: guest-poll останавливается при уходе с логина, сброс корзины после checkout,
  per-club депозит авторитетен + fallback username, удалён мёртвый DepositDisplay,
  WS UTF-8 по границе буфера, чистый переход тарифа. (`dotnet build` 0 ошибок.)
- Бэкенд: ComboSerializer nested-write (комбо больше не сохраняется пустым) + тест.
- Realtime/чат: channel layer авто-использует REDIS_URL (чат чинится на сервере).

### Осталось — требует ТВОЕГО решения/среды (не код «в лоб»)
- **Бизнес-модель:** выручка accrual-vs-cash (дашборд), нужны ли DISCOUNT-промокоды в кассе,
  цены по зонам/ночь/скидка оператора — продуктовые решения.
- **Миграции БД** (per-club уникальность промокода и т.п.) — рискованно вслепую, нужен Postgres + твоё «добро».
- **Гонки** — Postgres + конкурентный тест.
- **Безопасность киоска** (HTTPS-сертификат, пароль выхода, RCE-подпись) — стенд + решения.
...
- **✅ ИСПРАВЛЕНО (2026-06-18) — «нереальные данные» в аналитике (двойной счёт депозита):**
  выручка считала покупки тарифов/товаров **с депозита** (`payment_method="deposit"` и
  внутренние `[DEPOSIT]`-переводы) как новый доход — деньги уже учтены при пополнении.
  Завышались `revenue`/`income_per_pc`/`arpu`/`revenue_by_method.total` в 4 местах:
  `AnalyticsOverviewAPIView`, `AnalyticsShiftsAPIView`, `AnalyticsEquipmentAPIView`
  (`analytics.py`) и `DashboardStatsAPIView` (`dashboard.py`). Везде revenue теперь
  исключает `deposit`/`[DEPOSIT]` (паттерн как в `shift.py`/`dashboard.py`); строки
  `spending`/потребление и инфо-строка `deposit` оставлены. Проверено вживую: revenue
  37 000 → 0 на свежей базе. Регресс: `tests/test_analytics_revenue.py` (2 теста),
  сюита 61 зелёная. Чисто: visitors/clients/sales/games/transfers (только counts).
- **HIGH:** ~~депозит-топап учтён как TARIFF revenue (dashboard/analytics)~~ (✅ выше); Transfer
