...
no ClientSession, so the admin PC list showed the PC as free. The active_sessions_map now
surfaces prepaid guests AND seated registered clients, so «занят / играет» shows."""
import pytest
...
def test_guest_prepaid_shows_in_admin(api, make_club):
    ...
    assert p["active_session"] is not None, p
    assert p["active_session"]["client"] == "Гость"


@pytest.mark.django_db
def test_registered_seated_client_shows_in_admin(api, make_club, make_user):
    from apps.computers.models import Computer
    from apps.clubs.models import UserClubProfile
    club = make_club()
